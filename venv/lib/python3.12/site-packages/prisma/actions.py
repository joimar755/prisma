    ...
    async def query_first(
        ...
        *args
        ...
        user = await Venta.prisma().query_first(
            'SELECT * FROM Venta WHERE total = $1',
            1266032265.9325,
        )
        ```
        """
        ...
        *,
        ...
        total = await Venta.prisma().create_many(
            data=[
                {
                    # data to create a Venta record
                    'total': 127474245.94892,
                },
                {
                    # data to create a Venta record
                    'total': 1964990155.122889,
                },
            ],
            skip_duplicates=True,
        )
    ...
    async def query_raw(
        ...
        *args
        ...
        users = await Detalle.prisma().query_raw(
            'SELECT * FROM Detalle WHERE id = $1',
            1024265714,
        )
        ```
        """
    ...
    async def query_first(
        ...
        *args
        ...
        user = await Detalle.prisma().query_first(
            'SELECT * FROM Detalle WHERE cantidad = $1',
            872078403.187474,
        )
        ```
        """
    ...
    async def create(
        ...
        detalle = await Detalle.prisma().create(
            data={
                ...
                'cantidad': 916896761.76926,
                'Subtotal': 820312479.9272,
                'product_id': 344858293,
                'ventaId': 1121741130,
            },
        )
        ```
        ...
        *,
        ...
        total = await Detalle.prisma().create_many(
            data=[
                {
                    # data to create a Detalle record
                    'cantidad': 1495896251.20852,
                    'Subtotal': 860811569.166093,
                    'product_id': 525761943,
                    'ventaId': 736209796,
                },
                {
                    # data to create a Detalle record
                    'cantidad': 493907821.63968,
                    'Subtotal': 654007347.190526,
                    'product_id': 78746985,
                    'ventaId': 1398328302,
                },
            ],
